...
indicate the object of the postposition (described in
\cref{sec:postinfl}):
...
\obj{-ri} `\gl{pert}', some take \obj{-ti}. Unpossessed nouns generally
are unmarked, but some 15 nouns bear the suffix \obj{-të} `\gl{npert}'
...
\exref[]{onlypossessed}. A handful of nouns is marked with
\obj{-ri}/\obj{-ti} `\gl{pert}' when possessed and with \obj{-të}
`\gl{npert}' when not possessed \exref[]{diffpossessed}. Another handful
is unmarked when possessed and marked with \obj{-të} `\gl{npert}' when
not possessed \exref[]{suffunpossessed}. The fourth logical category,
...
\obj{a-} `\gl{2}' (\cref{tab:oldpossprefixes}).
...
optional \obj{ta-} in \gl{3}\textgreater{}\gl{3} scenarios.
...
\item
  allomorphy: none?
\item
  diachrony: from \obj{-jpë} `\gl{pst}.\gl{acnmlz}'
\end{itemize}
